...
document.getElementById("sellbirdCount").addEventListener("input",sellBirdTotalPrice );
...
function sellBirdTotalPrice(){
var optionBirdType = selectDropdownListValue("birdInputBird");
var sellCount = parseInt(document.getElementById("sellbirdCount").value);
sellPriceCalc(birdList, optionBirdType, sellCount);
...
return (myddl.options[myddl.selectedIndex].value);
	...
	function sellPriceCalc(vbirdList, vbirdType, vCount)
    {
		var birdExist = false;
		for(var i = 0; i < vbirdList.length; i++) {
			var Bird = vbirdList[i];
